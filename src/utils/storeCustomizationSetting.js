const storeCustomization = {
  setting: {
    navbar: {
      categories_menu_status: true,
      about_menu_status: true,
      contact_menu_status: true,
      offers_menu_status: true,
      term_and_condition_status: true,
      privacy_policy_status: true,
      faq_status: true,
      help_text: {
        pt: "Estamos disponíveis 24 horas por dia, Precisa de ajuda?",
      },
      categories: {
        pt: "Categorias",
      },
      about_us: {
        pt: "Sobre Nós",
      },
      contact_us: {
        pt: "Contato",
      },
      offers: {
        pt: "Ofertas",
      },
      faq: {
        pt: "FAQ",
      },
      privacy_policy: {
        pt: "Política de Privacidade",
      },
      term_and_condition: {
        pt: "Termos e Condições",
      },
      pages: {
        pt: "Páginas",
      },
      my_account: {
        pt: "Minha Conta",
      },
      login: {
        pt: "Entrar",
      },
      logout: {
        pt: "Sair",
      },
      checkout: {
        pt: "Finalizar",
      },
      phone: "+965 505 631288",
      logo: "https://res.cloudinary.com/ahossain/image/upload/v1697687802/settings/logo-light_hls14v.svg",
    },
    home: {
      coupon_status: true,
      featured_status: true,
      daily_needs_status: true,
      slider_width_status: false,
      promotion_banner_status: true,
      delivery_status: true,
      popular_products_status: true,
      discount_product_status: true,
      feature_promo_status: true,
      discount_coupon_code: ["WINTER21", "OCTOBER21"],
      place_holder_img: "",
      discount_title: {
        pt: "Código de cupom de desconto superior",
      },
      promotion_title: {
        pt: "100% Qualidade Organica Natural",
      },
      promotion_description: {
        pt: "Veja nossos produtos descontados aqui e obtenha um cupom de desconto especial",
      },
      promotion_button_name: {
        pt: "Compre agora",
      },
      promotion_button_link: "/search?category=breakfast",
      feature_title: {
        pt: "Categorias Destacadas",
      },
      feature_description: {
        pt: "Escolha seus produtos necessários desta categorias destacadas.",
      },
      feature_product_limit: 18,
      popular_title: {
        pt: "Produtos Populares para Compras Diarias",
      },
      popular_description: {
        pt: "Veja todos os nossos produtos populares nesta semana. Você pode escolher seus produtos necessários diariamente desta lista e obter alguma oferta especial com entrega gratuita.",
      },
      popular_product_limit: 18,
      quick_delivery_subtitle: {
        pt: "Entrega Rápida para Sua Casa",
      },
      quick_delivery_title: {
        pt: "Entrega Rápida para Sua Casa",
      },
      quick_delivery_description: {
        pt: "Há muitos produtos que você encontrará em nossa loja, escolha seu produto diário necessário em nossa loja JáPizza e ganhe algumas ofertas especiais. Veja nossos últimos produtos com desconto aqui e ganhe um desconto especial."
      },
      quick_delivery_button: {
        pt: "Baixar aplicativo",
      },
      quick_delivery_link: "#",
      quick_delivery_img:
        "https://res.cloudinary.com/ahossain/image/upload/v1697688032/settings/delivery-boy_rluuoq.webp",
      latest_discount_title: {
        pt: "Produtos Populares para Compras Diarias",
      },
      latest_discount_description: {
        pt: "Veja nossos produtos descontados abaixo. Escolha seus produtos necessários diariamente nesta lista e obtenha um desconto especial com entrega gratuita.",
      },
      latest_discount_product_limit: 18,
      daily_need_title: {
        pt: "Obtenha suas necessidades diárias em nossa loja JáPizza",
      },
      daily_need_description: {
        pt: "Há muitos produtos que vocês encontrarão em nossa loja, escolha seu produto diário necesario em nossa loja JáPizza e ganhe algumas ofertas especiais.",
      },
      daily_need_app_link: "https://www.apple.com/app-store/",
      daily_need_google_link:
        "https://play.google.com/store/games?utm_source=apac_med&utm_medium=hasem&utm_content=Jun0122&utm_campaign=Evergreen&pcampaignid=MKT-EDR-apac-lk-1003227-med-hasem-py-Evergreen-Jun0122-Text_Search_BKWS-BKWS%7CONSEM_kwid_43700071429441653_creativeid_600975795576_device_c&gclid=CjwKCAjwwo-WBhAMEiwAV4dybdy60tnQqCSnQ-cXShNnEcxmaBx2I6iwwc_WEqoA5sN9YSLJEXh9fBoC3u4QAvD_BwE&gclsrc=aw.ds",
      daily_need_img_left:
        "https://res.cloudinary.com/ahossain/image/upload/v1697688091/settings/app-download-img-left_s5n2zf.webp",
      daily_need_img_right:
        "https://res.cloudinary.com/ahossain/image/upload/v1697688091/settings/app-download-img_c7xqg4.webp",
      button1_img:
        "https://res.cloudinary.com/ahossain/image/upload/v1697688165/settings/app-store_cyyc0f.svg",
      button2_img:
        "https://res.cloudinary.com/ahossain/image/upload/v1697688167/settings/play-store_cavwua.svg",
    },
    about_us: {
      header_status: true,
      content_left_status: true,
      content_right_status: true,
      content_middle_status: true,
      founder_status: true,
      header_bg:
        "https://res.cloudinary.com/ahossain/image/upload/v1697439245/settings/yw3cd2xupqwqpqcbxv9l.jpg",
      content_right_img:
        "https://res.cloudinary.com/ahossain/image/upload/v1697439245/settings/v7g6gowiju0wanpwx70f.jpg",
      content_middle_Img:
        "https://res.cloudinary.com/ahossain/image/upload/v1697439195/settings/sl8vzvzm54jgzq6sphn2.jpg",
      founder_one_img:
        "https://res.cloudinary.com/ahossain/image/upload/v1697439688/settings/team-1_acjmv7.webp",
      founder_two_img:
        "https://res.cloudinary.com/ahossain/image/upload/v1697439689/settings/team-2_dw7zs1.webp",
      founder_three_img:
        "https://res.cloudinary.com/ahossain/image/upload/v1697439689/settings/team-3_ld3323.webp",
      founder_four_img:
        "https://res.cloudinary.com/ahossain/image/upload/v1697439689/settings/team-4_i7jvx7.webp",
      founder_five_img:
        "https://res.cloudinary.com/ahossain/image/upload/v1697439689/settings/team-5_ylyklw.webp",
      founder_six_img:
        "https://res.cloudinary.com/ahossain/image/upload/v1697439689/settings/team-6_gmlts4.webp",
      title: {
        pt: "Sobre Nós",
      },
      top_title: {
        pt: "Bem-vindo a nossa loja JáPizza",
      },
      top_description: {
        pt: "Aproveite holisticamente métricas paralelas e ROI funcional. Revolucione perfeitamente fontes internas ou orgânicas sem erros antes de cenários eficazes. Incentive progressivamente aplicativos de última geração para capital intelectual eficiente. Aproveite de forma confiável a participação mental distinta existente por meio de esquemas de ponta. Procrastine proativamente os paradigmas de construção de equipe, coordene o interno transparente total centrado no cliente. Abrace dinamicamente diversos paradigmas de atendimento ao cliente e base instalada. Aproveite de forma confiável experiências em toda a empresa para dados de ponta a ponta. Marque profissionalmente alinhamentos flexíveis e arquiteturas econômicas. Incentive entusiasticamente comunidades contínuas com métricas revolucionárias facilitadas perfeitamente com áreas temáticas estratégicas."
      },
      card_one_title: {
        pt: "10K",
      },
      card_one_sub: {
        pt: "Produtos Listados",
      },
      card_one_description: {
        pt: "Dinamicamente transformar parcerias de equipe orientadas após vertical",
      },
      card_two_title: {
        pt: "8K",
      },
      card_two_sub: {
        pt: "Bom Cliente",
      },
      card_two_description: {
        pt: "Dinamicamente transformar parcerias de equipe orientadas dopo vertical",
      },
      middle_description_one: {
        pt: "Aproveite holisticamente métricas paralelas e ROI funcional. Revolucione perfeitamente fontes internas ou orgânicas sem erros antes de cenários eficazes. Incentive progressivamente aplicativos de última geração para capital intelectual eficiente. Alavanque de forma confiável a participação mental distinta existente por meio de esquemas de ponta. Procrastine proativamente os paradigmas de construção de equipe, coordene o interno transparente total centrado no cliente. Reconceitualize energicamente a liderança global para redes de alta qualidade. Restaure de forma confiável uma matriz expandida de sistemas em vez de resultados precisos. Sinergize colaborativamente a largura de banda de backend sem funcionalidades 24 horas por dia, 7 dias por semana. Utilize de forma confiável ideias proativas, enquanto competências essenciais de mídia cruzada. Maximize exclusivamente as melhores práticas profissionais por meio de serviços de maximização de recursos. Arquitete convenientemente serviços da web entre unidades para imperativos de e-business."
      },
      middle_description_two: {
        pt: "Visualize apropriadamente dados orientados pelo mercado antes de cenários um-para-um. Produza colaborativamente o ROI multifuncional por meio de cadeias de suprimentos intuitivas. Aproveite com entusiasmo o valor revolucionário e serviços centrados em processos. Aproveite com competência informações intuitivas após mercados interoperáveis. Revolucione interativamente o valor à prova do futuro antes de fontes granulares. Adote dinamicamente diversos paradigmas de atendimento ao cliente e base instalada. Aproveite com credibilidade experiências em toda a empresa para dados de ponta a ponta. Marque profissionalmente alinhamentos flexíveis e arquiteturas econômicas. Incentive com entusiasmo comunidades integradas com métricas revolucionárias facilitadas perfeitamente com áreas temáticas estratégicas."
      },
      founder_title: {
        pt: "Nossa Equipe",
      },
      founder_description: {
        pt: "Somos imparciais e independentes e todos os dias criamos programas de fornecimento de backend reintermediate distintos e de classe mundial.",
      },
      founder_one_name: {
        pt: "Niamh Shea",
      },
      founder_one_sub: {
        pt: "Co-founder & Executive",
      },
      founder_two_name: {
        pt: "Orla Dwyer",
      },
      founder_two_sub: {
        pt: "Co-founder & Executive",
      },
      founder_three_name: {
        pt: "Danien James",
      },
      founder_three_sub: {
        pt: "Co-founder, Chairman",
      },
      founder_four_name: {
        pt: "Dara Frazier",
      },
      founder_four_sub: {
        pt: "Chief Strategy Officer",
      },
      founder_five_name: {
        pt: "Glenda Arvidson",
      },
      founder_five_sub: {
        pt: "HR Officer",
      },
      founder_six_name: {
        pt: "Melvin Davis ",
      },
      founder_six_sub: {
        pt: "Desenvolvedor Principal",
      },
    },
    contact_us: {
      header_status: true,
      email_box_status: true,
      call_box_status: true,
      address_box_status: true,
      left_col_status: true,
      form_status: true,
      header_bg:
        "https://res.cloudinary.com/ahossain/image/upload/v1697439245/settings/yw3cd2xupqwqpqcbxv9l.jpg",
      left_col_img:
        "https://res.cloudinary.com/ahossain/image/upload/v1697472924/settings/contact-us_zvwn2n.png",
      title: {
        pt: "Contate-nos",
      },
      email_box_title: {
        pt: "Envie-nos um e-mail",
      },
      email_box_email: {
        pt: "info@japizza.com",
      },
      email_box_text: {
        pt: "Cresça interativamente e fortaleça-se para uma vinculação total centrada em processos."
      },
      call_box_title: {
        pt: "Ligue para Nós",
      },
      call_box_phone: {
        pt: "029-00124667",
      },
      call_box_text: {
        pt: "Disseminar distintamente soluções focadas em cliques e argamassa ministate.",
      },
      address_box_title: {
        pt: "Localização",
      },
      address_box_address_one: {
        pt: "Rua dos Primarios, 123 - Portugal, 12345-678",
      },
      address_box_address_two: {
        pt: "Rua dos Primarios, 123 - Portugal, 12345-678",
      },
      address_box_address_three: {
        pt: "Portugal, 12345-678",
      },
      form_title: {
        pt: "Para qualquer suporte, envie sua consulta",
      },
      form_description: {
        pt: "Promova colaborativamente a convergência focada no cliente em relação aos alinhamentos direcionados ao cliente por meio de usuários estratégicos plagiados e infraestruturas padronizadas.",
      },
    },
    offers: {
      header_status: true,
      header_bg:
        "https://res.cloudinary.com/ahossain/image/upload/v1697439245/settings/yw3cd2xupqwqpqcbxv9l.jpg",
      title: {
        pt: "Mega Oferta",
      },
      coupon_code: ["JANEIRO25", "FEVEREIRO25", "MARCO25", "ABRIL25"],
    },
    privacy_policy: {
      status: true,
      header_bg:
        "https://res.cloudinary.com/ahossain/image/upload/v1697439245/settings/yw3cd2xupqwqpqcbxv9l.jpg",
      title: {
        pt: "Política de Privacidade",
      },
      description: {
        en: "<h1><strong>Last updated: February 15, 2022</strong></h1>\n<p>At jáPizza, accessible from jáPizza dot com, one of our main priorities is the privacy of our visitors. This Privacy Policy document contains types of information that is collected and recorded by jáPizza and how we use it. If you have additional questions or require more information about our Privacy Policy, do not hesitate to contact us.we may receive additional information about you such as your name, email address, phone number, the contents of the message and/or attachments you may send us, and any other information you may choose to provide. When you register for an Account, we may ask for your contact information, including items such as name, company name, address, email address, and telephone number.</p>\n<p>This Privacy Policy applies only to our online activities and is valid for visitors to our website with regards to the information that they shared and/or collect in jáPizza.we may receive additional information about you such as your name, email address, phone number, the contents of the message and/or attachments you may send us, we may ask for your contact information, including items such as name, company name, address, email address, and telephone number. This policy is not applicable to any information collected offline or via channels other than this website. Our Privacy Policy was created with the help of the Free Privacy Policy Generator.</p>\n<h1><strong>Consent</strong></h1>\n<p>By using our website, you hereby consent to our Privacy Policy and agree to its terms.</p>\n<h1><strong>Information we collect</strong></h1>\n<p>The personal information that you are asked to provide, and the reasons why you are asked to provide it, will be made clear to you at the point we ask you to provide your personal information. If you contact us directly, we may receive additional information about you such may choose to provide. When you register for an Account, we may ask for your contact information, including items such as name, company name, address, email address, and telephone number.</p>\n<p>Please note that the Company will not ask you to share any sensitive data or information via email or telephone. If you receive any such request by email or telephone, please do not respond/divulge any sensitive data or information and forward the information relating to the same to</p>\n<h1><strong>How we use your information</strong></h1>\n<p>We use the information we collect in various ways, including to:</p>\n<ol>\n<li>1. Provide, operate, and maintain our website, to provide you with updates and other information.</li>\n<li>2. Improve, personalize, and expand our website,and other information relating to the website.</li>\n<li>3. Understand and analyze how you use our website, to provide you with updates and other information relating to the website.</li>\n<li>4. Develop new products, services, features, and functionality,and other information relating to the website.</li>\n<li>5. Communicate with you, either directly or through one of our partners, including for customer service, to provide you with updates.</li>\n<li>6. Send you emails. To provide you with updates and other information relating to the website, and for marketing and promotional purposes</li>\n<li>7. Find and prevent fraud. To provide you with updates and other information relating to the website, and for marketing and promotional purposes</li>\n</ol>\n<h1><strong>Log Files</strong></h1>\n<p>jáPizza follows a standard procedure of using log files. These files log visitors when they visit websites. All hosting companies do this and a part of hosting. The information collected by log files include internet protocol (IP) addresses, browser type, Internet Service Provider (ISP), date and time stamp, referring/exit pages, and possibly the number of clicks. These are not linked to any information that is personally identifiable. The purpose of the information is for analyzing trends, administering the site, tracking users' movement on the website, and gathering demographic information.</p>\n<h1><strong>Advertising Partners Privacy Policies</strong></h1>\n<p>You may consult this list to find the Privacy Policy for each of the advertising partners of jáPizza. Third-party ad servers or ad networks uses technologies like cookies, JavaScript, or Web Beacons that are used in their respective advertisements and links that appear on jáPizza, which are sent directly to users' browser. They automatically receive your IP address when this occurs. These technologies are used to measure the effectiveness of their advertising campaigns and/or to personalize the advertising content that you see on websites that you visit. Note that jáPizza has no access to or control over these cookies that are used by third-party advertisers.</p>\n<h1><strong>Third Party Privacy Policies</strong></h1>\n<p>Third-party ad servers or ad networks uses technologies like cookies, JavaScript, or Web Beacons that are used in their respective advertisements and links that appear on jáPizza, which are sent directly to users' browser. They automatically receive your IP address when this occurs. These technologies are used to measure the effectiveness of their advertising campaigns and/or to personalize the advertising content that you see on websites that you visit. Note that jáPizza has no access to or control over these cookies that are used by third-party advertisers.</p>\n<p>jáPizza's Privacy Policy does not apply to other advertisers or websites. Thus, we are advising you to consult the respective Privacy Policies of these third-party ad servers for more detailed information. It may include their practices and instructions about how to opt-out of certain options. You can choose to disable cookies through your individual browser options. To know more detailed information about cookie management with specific web browsers, it can be found at the browsers' respective websites.</p>\n<h1><strong>CCPA Privacy Rights</strong></h1>\n<p>Under the CCPA, among other rights, California consumers have the right to: Request that a business that collects a consumer's personal data disclose the categories and specific pieces of personal data that a business has collected about consumers. Request that a business delete any personal data about the consumer that a business has collected. Request that a business that sells a consumer's personal data, not sell the consumer's personal data. If you make a request, we have one month to respond to you. If you would like to exercise any of these rights, please contact us.</p>\n<h1><strong>Children's Information</strong></h1>\n<p>Another part of our priority is adding protection for children while using the internet. We encourage parents and guardians to observe, participate in, and/or monitor and guide their online activity.Request that a business delete any personal data about the consumer that a business has collected. If you make a request, we have one month to respond to you. If you would like to exercise any of these rights, please contact us.</p>\n<p>jáPizza does not knowingly collect any Personal Identifiable Information from children under the age of 13. If you think that your child provided this kind of information on our website, we strongly encourage you to contact us immediately and we will do our best efforts to promptly remove such information from our records.</p>\n",
      },
    },
    term_and_condition: {
      status: true,
      header_bg:
        "https://res.cloudinary.com/ahossain/image/upload/v1697439245/settings/yw3cd2xupqwqpqcbxv9l.jpg",
      title: {
        pt: "Termos e Condições",
      },
      description: {
        en: '<h1><strong>Welcome to jáPizza!</strong></h1>\n<p>These terms and conditions outline the rules and regulations for the use of jáPizza\'s Website, located at https://jáPizza.com/. By accessing this website we assume you accept these terms and conditions. Do not continue to use jáPizza if you do not agree to take all of the terms and conditions stated on this page.</p>\n<p>The following terminology applies to these Terms and Conditions, Privacy Statement and Disclaimer Notice and all Agreements: Client, You and Your refers to you, the person log on this website and compliant to the Company’s terms and conditions. The Company, refers to our CompanyPartParties or Us refers to both the Client and ourselves. All terms refer to the offer, acceptance and consideration of payment necessary to undertake the process of our assistance to the Client in the most appropriate manner for the express purpose of meeting the Client’s needs in respect of provision of the Company’s stated services, in accordance with and subject to, prevailing law of Netherlands. Any use of the above terminology or other words in the singular, plural, capitalization and/or he/she or they, are taken as interchangeable and therefore as referring to same.</p>\n<h1><strong>Cookies</strong></h1>\n<p>We employ the use of cookies. By accessing jáPizza, you agreed to use cookies in agreement with the jáPizza\'s Privacy Policy. Most interactive websites use cookies to let us retrieve the user’s details for each visit. Cookies are used by our website to enable the functionality of certain areas to make it easier for people visiting our website. Some of our affiliate/advertising partners may also use cookies.</p>\n<h1><strong>License</strong></h1>\n<p>Unless otherwise stated, jáPizza and/or its licensors own the intellectual property rights for all material on jáPizza. All intellectual property rights are reserved. You may access this from jáPizza for your own personal use subjected to restrictions set in these terms and conditions.This Agreement shall begin on the date hereof. Our Terms and Conditions were created with the help of the Terms And Conditions Generator.<strong>You must not:</strong></p>\n<ul>\n<li>1. Identifiers (e.g. name, mailing address, email address, phone number, credit/debit card number)</li>\n<li>2. Characteristics of protected classifications (e.g. gender, age)</li>\n<li>3. Commercial information (e.g. products or services purchased, purchase history)</li>\n<li>4. Internet or other electronic network activity (e.g. browse or search history)</li>\n<li>5. Geo location data (e.g. latitude or longitude)</li>\n<li>6. Audio, electronic, visual, or similar information (e.g. recording of Guest service calls)</li>\n<li>7. Inferences drawn from any of the above (e.g. preferences or characteristics)</li>\n</ul>\n<p>Parts of this website offer an opportunity for users to post and exchange opinions and information in certain areas of the website. jáPizza does not filter, edit, publish or review Comments prior to their presence on the website. Comments do not reflect the views and opinions of jáPizza,its agents and/or affiliates. Comments reflect the views and opinions of the person who post their views and opinions. To the extent permitted by applicable laws, jáPizza shall not be liable for the Comments or for any liability, damages or expenses caused and/or suffered as a result of any use of and/or posting of and/or appearance of the Comments on this website.</p>\n<h1><strong>Content Liability</strong></h1>\n<p>We shall not be hold responsible for any content that appears on your Website. You agree to protect and defend us against all claims that is rising on your Website. No link(s) should appear on any Website that may be interpreted as libelous, obscene or criminal, or which infringes, otherwise violates, or advocates the infringement or other violation of, any third party rights.Without prior approval and written permission, you may not create frames around our Webpages that alter in any way the visual presentation or appearance of our Website.</p>\n<h1><strong>Your Privacy</strong></h1>\n<p>Please read <a href="https://japizza-loja.vercel.app/privacy-policy" target="_self">Privacy Policy</a></p>\n<h1><strong>Reservation of Rights</strong></h1>\n<p>We reserve the right to request that you remove all links or any particular link to our Website. You approve to immediately remove all links to our Website upon request. We also reserve the right to amen these terms and conditions and it’s linking policy at any time. By continuously linking to our Website, you agree to be bound to and follow these linking terms and conditions.</p>\n<h1><strong>Disclaimer</strong></h1>\n<p>To the maximum extent permitted by applicable law, we exclude all representations, warranties and conditions relating to our website and the use of this website. Nothing in this disclaimer will:</p>\n<ul>\n<li>1. limit or exclude our or your liability for death or personal injury;</li>\n<li>2. limit or exclude our or your liability for fraud or fraudulent misrepresentation;</li>\n<li>3. limit any of our or your liabilities in any way that is not permitted under applicable law; or</li>\n<li>4. exclude any of our or your liabilities that may not be excluded under applicable law.</li>\n</ul>\n<p>The limitations and prohibitions of liability set in this Section and elsewhere in this disclaimer: (a) are subject to the preceding paragraph; and (b) govern all liabilities arising under the disclaimer, including liabilities arising in contract, in tort and for breach of statutory duty. As long as the website and the information and services on the website are provided free of charge, we will not be liable for any loss or damage of any nature.</p>\n',
      },
    },
    faq: {
      page_status: true,
      leftcol_status: true,
      rightcol_status: true,
      header_bg:
        "https://res.cloudinary.com/ahossain/image/upload/v1697439245/settings/yw3cd2xupqwqpqcbxv9l.jpg",
      left_img:
        "https://res.cloudinary.com/ahossain/image/upload/v1697687955/settings/faq_qr1y1h.svg",
      title: {
        pt: "Perguntas frequentes",
      },
      faq_one: {
        pt: "Como o JáPizza funciona?",
      },
      description_one: {
        pt: "Sim. Você pode cancelar sua assinatura a qualquer momento. Sua assinatura continuará ativa até o fim do seu prazo atual (mês ou ano), mas não será renovada automaticamente. A menos que você exclua sua conta manualmente, sua conta e todos os dados serão excluídos 60 dias a partir do dia em que sua assinatura se tornar inativa."
      },
      faq_two: {
        pt: "Posso cancelar minha assinatura a qualquer momento?"
      },
      description_two: {
        pt: "Inicie distintamente canais sem erros com ROI altamente eficiente. Visualize intrinsecamente dados de classe mundial por meio das melhores práticas da categoria. Habilite com eficiência varejistas eletrônicos capacitados após serviços entre unidades. Acelere exclusivamente varejistas eletrônicos sem interrupções por meio de interfaces cooperativas. Miocardine monotonamente os meta-serviços direcionados ao cliente, enquanto cenários sem erros."
      },
      faq_three: {
        pt: "Qual método de pagamento você deve aceitar?"
      },
      description_three: {
        pt: "Envolva holisticamente nichos de mercado pegajosos antes da colaboração colaborativa e compartilhamento de ideias. Facilite fosforescentemente aplicações paralelas com imperativos únicos. Plagie proativamente entregas funcionalizadas por meio de soluções baratas. Adote colaborativamente infomediários habilitados para a web em vez de procedimentos de teste diversos."
      },
      faq_four: {
        pt: "Posso cancelar minha assinatura a qualquer momento?"
      },
      description_four: {
        pt: "Impacte continuamente imperativos contínuos para as melhores práticas da categoria. Facilite fosforescentemente aplicações paralelas com imperativos exclusivos. Plagie proativamente entregas funcionalizadas por meio de soluções baratas. Adote colaborativamente infomediários habilitados para a web em vez de procedimentos de teste diversos."
      },
      faq_five: {
        pt: "O que é JáPizza EC2 auto scaling?",
      },
      description_five: {
        pt: "Impacte continuamente imperativos contínuos para as melhores práticas da categoria. Facilite fosforescentemente aplicações paralelas com imperativos exclusivos. Plagie proativamente entregas funcionalizadas por meio de soluções baratas. Adote colaborativamente infomediários habilitados para a web em vez de procedimentos de teste diversos."
      },
      faq_six: {
        pt: "Quais são os benefícios de usar o afiliado jáPizza?"
      },
      description_six: {
        pt: "Continually impact seamless imperatives for best-of-breed best practices. Phosfluorescently facilitate parallel applications with unique imperatives. Proactively plagiarize functionalized deliverables via inexpensive solutions. Collaboratively embrace web-enabled infomediaries rather than diverse testing procedures."
      },
      faq_seven: {
        pt: "O que é uma configuração de produtos afiliados?",
      },
      description_seven: {
        pt: "Sim. Você pode cancelar sua assinatura a qualquer momento. Sua assinatura continuará ativa até o fim do seu prazo atual (mês ou ano), mas não será renovada automaticamente. A menos que você exclua sua conta manualmente, sua conta e todos os dados serão excluídos 60 dias a partir do dia em que sua assinatura se tornar inativa."
      },
      faq_eight: {
        pt: "O que é o gerenciamento de veículos e como ele difere da escala dinâmica?",
      },
      description_eight: {
        pt: "Inicie distintamente canais sem erros com ROI altamente eficiente. Visualize intrinsecamente dados de classe mundial por meio das melhores práticas da categoria. Habilite com eficiência varejistas eletrônicos capacitados após serviços entre unidades. Acelere exclusivamente varejistas eletrônicos sem interrupções por meio de interfaces cooperativas. Miocardine monotonamente os meta-serviços direcionados ao cliente, enquanto cenários sem erros."
      },
    },
    slider: {
      left_right_arrow: false,
      bottom_dots: true,
      both_slider: false,
      first_img:
        "https://res.cloudinary.com/ahossain/image/upload/v1697688491/settings/slider-1_rl8qdc.jpg",
      first_title: {
        pt: "Os Melhores Produtos de Qualidade Garantidos!",
      },
      first_description: {
        pt: "Os Melhores Produtos de Qualidade Garantidos!",
      },
      first_button: {
        pt: "Compre Agora",
      },
      first_link: "/search?category=milk-dairy",
      second_img:
        "https://res.cloudinary.com/ahossain/image/upload/v1697688491/settings/slider-2_o6aezc.jpg",
      second_title: {
        pt: "Melhores Lojas de Mercado",
      },
      second_description: {
        pt: "Agregue rapidamente redes capacitadas após o surgimento de produtos..."
      },
      second_button: {
        pt: "Compre Agora",
      },
      second_link: "/search?category=fish-meat",
      third_img:
        "https://res.cloudinary.com/ahossain/image/upload/v1697688492/settings/slider-3_iw4nnf.jpg",
      third_title: {
        pt: "Os Melhores Produtos de Qualidade Garantidos!",
      },
      third_description: {
        pt: "Os Melhores Produtos de Qualidade Garantidos!",
      },
      third_button: {
        pt: "Compre Agora",
      },
      third_link: "/search?category=fruits-vegetable",
      four_img: "https://i.postimg.cc/rscqZJNz/slider-1.webp",
      four_title: {
        pt: "Os Melhores Produtos de Qualidade Garantidos!",
      },
      four_description: {
        pt: "Os Melhores Produtos de Qualidade Garantidos!",
      },
      four_button: {
        pt: "Compre Agora",
      },
      four_link: "/search?category=fruits-vegetable",
      five_img:
        "https://res.cloudinary.com/ahossain/image/upload/v1697688491/settings/slider-2_o6aezc.jpg",
      five_title: {
        pt: "Melhores Lojas de Mercado",
      },
      five_description: {
        pt: "Agregue rapidamente redes capacitadas depois do surgimento de produtos...",
      },
      five_button: {
        pt: "Compre Agora",
      },
      five_link: "/search?category=fish-meat",
    },
    checkout: {
      personal_details: {
        pt: "Detalhes pessoais",
      },
      first_name: {
        pt: "Primeiro Nome",
      },
      last_name: {
        pt: "Sobrenome",
      },
      email_address: {
        pt: "Endereço de E-mail",
      },
      checkout_phone: {
        pt: "Telefone",
      },
      shipping_details: {
        pt: "Detalhes de entrega",
      },
      street_address: {
        pt: "Endereço",
      },
      city: {
        pt: "Cidade",
      },
      country: {
        pt: "Pais",
      },
      zip_code: {
        pt: "CEP",
      },
      shipping_cost: {
        pt: "Custo de envio",
      },
      shipping_name_one: {
        pt: "FedEx",
      },
      shipping_one_cost: 60,
      shipping_one_desc: {
        pt: "Entrega: Hoje Custo :",
      },
      shipping_name_two: {
        pt: "UPS",
      },
      shipping_two_desc: {
        pt: "Entrega: 7 Dias Custo :",
      },
      shipping_two_cost: 20,
      payment_method: {
        pt: "Forma de pagamento",
      },
      continue_button: {
        pt: "Continuar Comprando",
      },
      confirm_button: {
        pt: "Confirmar Pedido",
      },
      order_summary: {
        pt: "Resumo do Pedido",
      },
      apply_button: {
        pt: "Aplicar",
      },
      sub_total: {
        pt: "Subtotal",
      },
      discount: {
        pt: "Desconto",
      },
      total_cost: {
        pt: "Custo Total",
      },
    },
    dashboard: {
      invoice_message_first: {
        pt: "Obrigado",
      },
      invoice_message_last: {
        pt: "Seu pedido foi recebido!",
      },
      print_button: {
        pt: "Imprimir Fatura",
      },
      download_button: {
        pt: "Baixar Fatura",
      },
      dashboard_title: {
        pt: "Painel de Controle",
      },
      total_order: {
        pt: "Pedidos Totais",
      },
      pending_order: {
        pt: "Pedidos Pendentes",
      },
      processing_order: {
        pt: "Pedidos em Processo",
      },
      complete_order: {
        pt: "Pedidos Completos",
      },
      recent_order: {
        pt: "Pedidos Recentes",
      },
      my_order: {
        pt: "Pedidos",
      },
      update_profile: {
        pt: "Atualizar Perfil",
      },
      full_name: {
        pt: "Nome Completo",
      },
      address: {
        pt: "Endereço",
      },
      user_phone: {
        pt: "Telefone/Mobile",
      },
      user_email: {
        pt: "Endereço de E-mail",
      },
      update_button: {
        pt: "Atualizar Perfil",
      },
      current_password: {
        pt: "Senha Atual",
      },
      new_password: {
        pt: "Nova Senha",
      },
      change_password: {
        pt: "Alterar Senha",
      },
    },
    footer: {
      promo_status: true,
      block1_status: true,
      block2_status: true,
      block3_status: true,
      block4_status: true,
      payment_method_status: true,
      bottom_contact_status: true,
      social_links_status: true,
      shipping_card: {
        pt: "Envio Gratuito a partir de R$ 500,00",
      },
      support_card: {
        pt: "Suporte 24/7 24 Horas",
      },
      payment_card: {
        pt: "Pagamento Seguro, 100% Seguro",
      },
      offer_card: {
        pt: "Oferta de 20% Off",
      },
      block1_title: {
        pt: "Empresa",
      },
      block1_sub_title1: {
        pt: "Sobre Nós",
      },
      block1_sub_link1: "/about-us",
      block1_sub_title2: {
        pt: "Contate-nos",
      },
      block1_sub_link2: "/contact-us",
      block1_sub_title3: {
        pt: "Vagas",
      },
      block1_sub_link3: "#",
      block1_sub_title4: {
        pt: "Notícias",
      },
      block1_sub_link4: "#",
      block2_title: {
        pt: "Notícias",
      },
      block2_sub_title1: {
        pt: "Peixe e Carnes",
      },
      block2_sub_link1: "/search?category=fish-meat",
      block2_sub_title2: {
        pt: "Bebidas",
      },
      block2_sub_link2: "/search?category=drinks",
      block2_sub_title3: {
        pt: "Leite e Laticínios",
      },
      block2_sub_link3: "/search?category=milk-dairy",
      block2_sub_title4: {
        pt: "Beleza e Saúde",
      },
      block2_sub_link4: "/search?category=beauty-health",
      block3_title: {
        pt: "Minha Conta",
      },
      block3_sub_title1: {
        pt: "Painel de Controle",
      },
      block3_sub_link1: "/user/dashboard",
      block3_sub_title2: {
        pt: "Meus Pedidos",
      },
      block3_sub_link2: "/user/my-orders",
      block3_sub_title3: {
        pt: "Pedidos Recentes",
      },
      block3_sub_link3: "/user/dashboard",
      block3_sub_title4: {
        pt: "Atualizar Perfil",
      },
      block3_sub_link4: "/user/update-profile",
      block4_logo:
        "https://res.cloudinary.com/ahossain/image/upload/v1697688576/settings/logo-color_el4zmy.svg",
      block4_address: {
        pt: "R. das Escadinhas 5, 8500-132, Portugal"
      },
      block4_phone: "02.356.1666",
      block4_email: "ccruidk@test.com",
      social_facebook: "https://www.facebook.com/japizzaportugal",
      social_instagram: "https://www.instagram.com/japizzapt",
      social_whatsapp: "https://api.whatsapp.com/send?phone=351912827537&text=%F0%9F%91%8B%20Ol%C3%A1%2C%20venho%20de%20https%3A%2F%2Fjapizza.ola.click%20%0AEu%20quero%20fazer%20o%20pr%C3%B3ximo%20pedido%3A%0A",
      payment_method_img:
        "https://res.cloudinary.com/ahossain/image/upload/v1697688607/settings/payment-logo_qhslgz.webp",
      bottom_contact: "+6599887766",
    },
    slug: {
      right_box_status: true,
      card_description_one: {
        pt: "Envio Gratuito para todos os pedidos acima de € 100",
      },
      card_description_two: {
        pt: "Entrega em domicílio em até 1 hora",
      },
      card_description_three: {
        pt: "Pagamento na entrega disponível"
      },
      card_description_four: {
        pt: "7 dias de garantia de devolução de dinheiro",
      },
      card_description_five: {
        pt: "Garantia não disponível para este item",
      },
      card_description_six: {
        pt: "Garantia de 100% orgânicos de produtos naturais.",
      },
      card_description_seven: {
        pt: "Entrega de nossa ponta de entrega para várias regiões em portugal.",
      },
    },
    seo: {
      favicon:
        "https://res.cloudinary.com/ahossain/image/upload/v1700903173/undefined/favicon.png",
      meta_description:
        "A melhor pizza de Mexilhoeira Grande",
      meta_img:
        "https://res.cloudinary.com/ahossain/image/upload/v1636729752/facebook-page_j7alju.png",
      meta_keywords: "e-commerce, online store, pizza, pizzaria, mexilhoeira grande",
      meta_title:
        "JáPizza - A melhor pizza de Mexilhoeira Grande",
      meta_url: "https://frontend-loja-one.vercel.app/",
    },
  },
  name: "storeCustomizationSetting",
};

export { storeCustomization };
